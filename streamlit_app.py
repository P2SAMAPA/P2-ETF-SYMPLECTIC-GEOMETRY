...
st.set_page_config(page_title="Symplectic Geometry – Poincaré Section Entropy", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Symplectic Geometry – Poincaré Section Entropy</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Phase space embedding | Poincaré section | First return map entropy</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Symplectic")
...
st.sidebar.markdown(f"**Entropy bins:** {config.N_BINS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'symplectic_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Poincaré entropy: {etf['symp_score_norm']:.3f}</p>
...
with tab1:
    st.header("🌀 Top ETFs by Poincaré Section Entropy (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Symplectic geometry** studies phase space flows (position = return, momentum = return difference).
        - The **Poincaré section** captures the dynamics at a fixed momentum (e.g., zero).
        - The **first return map** records successive crossings of the section.
        - **Entropy** of the return map quantifies chaos: high entropy = chaotic/unpredictable, low entropy = regular/trending.
        - Low entropy → ETF dynamics are predictable → potential trend‑following signal.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the Poincaré section entropy.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Symplectic Geometry | Poincaré section entropy for ETF dynamics")
